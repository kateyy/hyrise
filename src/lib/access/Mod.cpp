  ...
  if(getVType() == 0){
    for(size_t row = 0; row < in->size(); row++) {
      resultTable->setValue<hyrise_int_t>(0, row, 
          in->getValue<int>(_field_definition[0], row) % int(getDivisor())
      );
    }
  }else{
    for(size_t row = 0; row < in->size(); row++) {
      resultTable->setValue<hyrise_float_t>(0, row, std::fmod(
          in->getValue<float>(_field_definition[0], row),
          getDivisor()
      ));
    }
  }
...
void Mod::setVType(const int &vtype){
  _vtype = vtype;
}

void Mod::setColName(const std::string &colName){
  _colName = colName;
}

float Mod::getDivisor()const {
  return _divisor;
}

int Mod::getVType() const {
  return _vtype;
}

std::string Mod::getColName() const {
  return _colName;
}
